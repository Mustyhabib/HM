...
class Config:
    ...
    # Multi-provider BYOK (2026-08-22): provider + model are resolved per-user
    # at run time from the providers catalog (providers.py / llm_providers.json).
    # These fields are kept for observability / logging only — they are NOT
    # passed to TradiRunner. The catalog's default_model for each provider is
    # used instead.
